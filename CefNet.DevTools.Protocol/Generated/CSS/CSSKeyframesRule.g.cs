//
// DO NOT MODIFY. THIS IS AUTOMATICALLY GENERATED FILE.
//
#nullable enable
#pragma warning disable CS8618 // Non-nullable field must contain a non-null value when exiting constructor. Consider declaring as nullable.
using System;
using System.Collections.Generic;

namespace CefNet.DevTools.Protocol.CSS
{
    /// <summary>CSS keyframes rule representation.</summary>
    public sealed class CSSKeyframesRule
    {
        /// <summary>Animation name.</summary>
        public CefNet.DevTools.Protocol.CSS.Value AnimationName { get; set; }

        /// <summary>List of keyframes.</summary>
        public System.Collections.Generic.List<CefNet.DevTools.Protocol.CSS.CSSKeyframeRule> Keyframes { get; set; }
    }
}
